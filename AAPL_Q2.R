######## PROJECT HFT ##############
### Nijat Aliyev and Umid Suleymanli AAPL  #############
      ...
      for(slowEMA in 60)
      {
        for(volat.sd in 90)
        {
          for(m_ in 1)
          {
            ...
            signalEMA.values <- EMA(coredata(na.locf(AAPL$AAPL)), signalEMA)
            slowEMA.values <- EMA(coredata(na.locf(AAPL$AAPL)), slowEMA)
            volat.sd.values <- runsd(coredata(na.locf(AAPL$AAPL)), volat.sd, 
                                     endrule = "NA", align = "right")
            
            # put missing values whenever the original price is missing
            signalEMA.values[is.na(AAPL$AAPL)] <- NA
            slowEMA.values[is.na(AAPL$AAPL)] <- NA
            volat.sd.values[is.na(AAPL$AAPL)] <- NA
            ...
            # gross pnl
            pnl.gross.mom <- ifelse(is.na(pos.mom * diff.xts(coredata(AAPL$AAPL))),
                                    0, pos.mom * diff.xts(coredata(AAPL$AAPL)) )
            ...
            # net pnl
            pnl.net.mom <- pnl.gross.mom - ntrans * 1 # 1$ per transaction of AAPL
            pnl.net.mr <- pnl.gross.mr - ntrans * 1 # 1$ per transaction of AAPL
...
write.csv(summary.all.breakout_mr, file="sum_mr_AAPL_Q2.csv", row.names = F)
write.csv(summary.all.breakout_mom, file="sum_mom_AAPL_Q2.csv", row.names = F)
